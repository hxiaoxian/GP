...
public class NodeImpl<T> implements Node<T>, NodeChange {
    ...
    public synchronized KVResponse handleClientRequest(KVRequest request) {
        if (request.getType() == 0) {
            LOGGER.info("客户端 Put 请求, key : [{}], value : [{}]", request.getKey(), request.getValue());
        } else if (request.getType() == 1) {
            LOGGER.info("客户端 Get 请求, key : [{}]", request.getKey());
        } else if (request.getType() == 2) {
            LOGGER.info("客户端 Del 请求, key : [{}]", request.getKey());
        }
        if (status != LEADER) {
            LOGGER.info("将请求重定向到 leader节点 : [{}]", peerSet.getLeader());
        ...
        LOGGER.info("预提交到本地日志, 日志 : {}, 日志index : {}", logEntry, logEntry.getIndex());
        ...
        if (success.get() >= (count / 2)) {
            ...
            if (request.getType() == KVRequest.PUT) {
                ...
                LOGGER.info("状态机写入成功,  logEntry : {}  key : {}  value : {}", logEntry, request.getKey(), request.getValue());
                // 返回成功.
                ...
                lastApplied = commitIndex;
                LOGGER.info("状态机删除成功,  logEntry : {}  key : {}  value : {}", logEntry, request.getKey(), request.getValue());
                // 返回成功.
                return KVResponse.ok();
            }
            ...
            LOGGER.warn("状态机应用失败,  logEntry : {}", logEntry);
            ...
            while (end - start < 20 * 1000L) {
                ...
                try {
                    ...
                    if (result != null && result.isSuccess()) {
                        LOGGER.info("向follower发送日志 , follower=[{}], 日志=[{}]", peer, entryParam.getEntries());
                        // update 这两个追踪值
                        ...
                        if (result.getTerm() > currentTerm) {
                            LOGGER.warn("follower [{}] 任期 [{}] 更大, leader 任期是 [{}], 因此 leader 将成为 follower",
                            ...
                            LOGGER.warn("follower {} nextIndex 不匹配, 将会减小nextIndex并重新发送附加日志请求, nextIndex : [{}]", peer.getAddr(),
                                    nextIndex);
                            // 重来, 直到成功.
                        }
                    }
    ...
    class HeartBeatTask implements Runnable {
        ...
        public void run() {
            ...
            LOGGER.info("==========================");
            for (Peer peer : peerSet.getPeersWithOutSelf()) {
                LOGGER.info("向结点 [{}] 发送心跳", peer.getAddr());
            }
            ...
            for (Peer peer : peerSet.getPeersWithOutSelf()) {
                ...
                RaftThreadPool.execute(() -> {
                    try {
                        ...
                        if (term > currentTerm) {
                            LOGGER.error("我将成为follower, 对方的任期={}, 我的任期={}", term, currentTerm);
                            currentTerm = term;
                            votedFor = "";
                            status = NodeStatus.FOLLOWER;
                        }
    ...
    class ElectionTask implements Runnable {
        ...
        public void run() {
            ...
            LOGGER.info("结点 [{}] 成为 Candidate 并开始竞选 Leader, 原任期={}, 自增后任期={}, 最新的日志 : [{}]",
            ...
            LOGGER.info("========== 主节点向其他结点发送选举请求 ==========");

            // 发送请求
            for (Peer peer : peers) {
                LOGGER.info("发送选举请求 : {}", peer);
            ...
            LOGGER.info("结点 {} 可能成为leader , 票数 = {} , 目前状态 : {}", peerSet.getSelf(), success, "Candidate");
            // 如果投票期间,有其他服务器发送 appendEntry , 就可能变成 follower ,这时,应该停止.
            if (status == NodeStatus.FOLLOWER) {
                LOGGER.info("结点 {} 接收到主节点的心跳 , 变为 Follower ", peerSet.getSelf());
                return;
            }
            // 加上自身.
